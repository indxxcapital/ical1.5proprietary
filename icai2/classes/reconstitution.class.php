...
$newData=$this->getallDataofTicker($security['2']);
		...
		foreach($data as $key=> $security)
		{
			...
			$outputArray[$key]['ticker']=$newData[$security['2']]['ticker'];
			$outputArray[$key]['isin']=$security['2'];
			$outputArray[$key]['name']=$newData[$security['2']]['name'];
			$outputArray[$key]['curr']=$newData[$security['2']]['curr'];
			$outputArray[$key]['divcurr']=$newData[$security['2']]['divcurr'];
			$outputArray[$key]['sedol']=$newData[$security['2']]['sedol'];
			$outputArray[$key]['cusip']=$newData[$security['2']]['cusip'];
			$outputArray[$key]['countryname']=$newData[$security['2']]['countryname'];
			$outputArray[$key]['sector']=$newData[$security['2']]['sector'];
			$outputArray[$key]['industry']=$newData[$security['2']]['industry'];
			$outputArray[$key]['subindustry']=$newData[$security['2']]['subindustry'];
...
fclose($output) or die("Can't close php://output");
	...
	{
	$indxxdata2=array();
	$indxxdata=$this->db->getResult("select distinct(isin),ticker,name,curr,divcurr,sedol,cusip,countryname,sector,industry,subindustry from tbl_indxx_ticker union select distinct(isin),ticker,name,curr,divcurr,sedol,cusip,countryname,sector,industry,subindustry from tbl_indxx_ticker_temp",true);
	//	if(empty($indxxdata))
	//	$indxxdata=$this->db->getResult("select * from tbl_indxx_ticker_temp where isin='".$isin."'",false,1);
	if(!empty($indxxdata))
	{
		foreach($indxxdata as $key=>$value)
		{$indxxdata2[$value['isin']]=$value;
		}
	}	
	return $indxxdata2;
