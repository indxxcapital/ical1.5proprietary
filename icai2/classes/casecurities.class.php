...
//	$this->addfield();
	...
	$this->_baseTemplate="inner-template";
		$this->_bodyTemplate="casecurities/download";
		$this->_title=$this->siteconfig->site_title;
		$this->_meta_description=$this->siteconfig->default_meta_description;
		$this->_meta_keywords=$this->siteconfig->default_meta_keyword;
		$this->smarty->assign('pagetitle','Download Securities');
		$this->smarty->assign('bredcrumssubtitle','Download Securities');
		//$this->addfield();
		...
		$data =$this->db->getResult("select tbl_indxx.code,ticker,isin,tbl_indxx_ticker.curr,divcurr,sedol,cusip,countryname,weight from tbl_indxx_ticker left join tbl_indxx on tbl_indxx.id=tbl_indxx_ticker.indxx_id where tbl_indxx.id in (".$ids."); ");
		//$this->pr($data,true);
		//exit;
		$output = fopen("php://output",'w') or die("Can't open php://output");
header("Content-Type:application/csv"); 
header("Content-Disposition:attachment;filename=updatedsecurities.csv"); 
fputcsv($output, array('code','ticker','isin','curr','divcurr','sedol','cusip','countryname','weight'));
